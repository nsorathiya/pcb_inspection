...
test('loads the demo workspace and proves session-only engineering alignment and native pixel measurements', async ({ page, networkRecords }) => {
  ...
  const nativeHeightBeforeDisplayChange = await page.getByText('Native height', { exact: true }).locator('..').locator('dd').textContent()
  const nativeMinimum = Number(await page.getByLabel('Height display minimum').inputValue())
  const nativeMaximum = Number(await page.getByLabel('Height display maximum').inputValue())
  const displayMinimum = nativeMinimum + (nativeMaximum - nativeMinimum) * 0.2
  const displayMaximum = nativeMinimum + (nativeMaximum - nativeMinimum) * 0.8
  await page.getByLabel('Height palette').selectOption('viridis-like')
  await page.getByLabel('Height display minimum').fill(String(displayMinimum))
  await page.getByLabel('Height display maximum').fill(String(displayMaximum))
  await page.getByRole('button', { name: 'Apply display range' }).click()
  await expect(page.getByText('Display range changes only the derived colour view. Native height values remain unchanged.')).toBeVisible()
  await expect(page.getByRole('status', { name: 'Engineering workspace status' })).toContainText('Palette viridis-like')
  await page.getByRole('checkbox', { name: /Show invalid pixels/ }).check()
  await expect(page.getByRole('status', { name: 'Engineering workspace status' })).toContainText('Invalid shown')
  await page.getByRole('button', { name: 'Sample values' }).click()
  await expect(page.getByText('Native height', { exact: true }).locator('..').locator('dd')).toHaveText(nativeHeightBeforeDisplayChange ?? '')
  ...
  await expect(page.locator('[data-measurement-kind="LINE"]')).toContainText('dx')
  await expect(page.locator('[data-measurement-kind="LINE"]')).toContainText('direction')
  ...
  await expect(page.locator('.roi-range-marker')).toBeVisible()

  page.once('dialog', (dialog) => dialog.accept())
  await page.getByRole('button', { name: 'Reset Engineering Session' }).click()
  await expect(page.getByLabel('Height palette')).toHaveValue('grayscale')
  await expect(page.getByRole('checkbox', { name: /Show invalid pixels/ })).not.toBeChecked()
  await expect(page.getByLabel('Height display minimum')).toHaveValue(String(nativeMinimum))
  await expect(page.getByLabel('Height display maximum')).toHaveValue(String(nativeMaximum))
  await expect(page.getByLabel('Zoom level')).toHaveText('100%')
